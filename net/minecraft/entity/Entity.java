...
import wtf.expensive.event.impl.player.EventPostMove;
import wtf.expensive.modules.impl.combat.HitBoxModule;
...
public abstract class Entity implements ICommandSender {
    ...
    public static double Getmotionx, Getmotiony, Getmotionz;
    ...
    public double getDistanceToVec3d(Vec3d vec3) {
        return this.getDistanceAtEye(vec3.x, vec3.y, vec3.z);
    }

    public double getDistanceAtEye(double x, double y, double z) {
        double d0 = this.posX - x;
        double d1 = this.posY + this.getEyeHeight() - y;
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        double preX = posX, preZ = posZ;
        AxisAlignedBB before = this.getEntityBoundingBox().addCoord(0, 0, 0);
        if (this.noClip) {
            ...
            if (((x == MoverType.SELF || x == MoverType.PLAYER) && this.onGround && this.isSneaking())) {
            ...
            Vec3d collisionOffset = new Vec3d(0, 0, 0);
            Vec3d backUp = new Vec3d(posX, posY, posZ);
            ...
            if (this instanceof EntityPlayerSP) {
                double bpMX = p_70091_2_, bpMY = p_70091_4_, bpMZ = p_70091_6_;
                double predictX = posX, predictY = posY, predictZ = posZ;
                if (p_70091_4_ != 0.0D) {
                    int k = 0;
                    for (int l = list1.size(); k < l; ++k) {
                        p_70091_4_ = ((AxisAlignedBB) list1.get(k)).calculateYOffset(this.getEntityBoundingBox(), p_70091_4_);
                    }
                    predictY = predictY + p_70091_4_;
                }
                ...
                predictGround = isCollidedVertically && d3 < 0.0D;
                p_70091_2_ = bpMX;
                p_70091_4_ = bpMY;
                p_70091_6_ = bpMZ;
                EventMove move = new EventMove(new Vec3d(this.posX, this.posY, this.posZ), new Vec3d(predictX, predictY, predictZ), new Vec3d(p_70091_2_, p_70091_4_, p_70091_6_), collisionOffset, predictGround, isCollidedHorizontally, isCollidedVertically, before);
                EventManager.dispatchEvent(move);
                ignoreVertical = move.isIgnoreVertical();
                ignoreHorizontal = move.isIgnoreHorizontal();
                p_70091_2_ = move.motion().x;
                p_70091_4_ = move.motion().y;
                p_70091_6_ = move.motion().z;
                d2 = move.motion().x;
                d3 = move.motion().y;
                d4 = move.motion().z;
            }
            ...
            if (this instanceof EntityPlayerSP) {
                List<AxisAlignedBB> bb = this.world.getCollisionBoxes2(this, this.getEntityBoundingBox().addCoord(p_70091_2_, p_70091_4_, p_70091_6_).offset(collisionOffset.x, collisionOffset.y, collisionOffset.z));
                if (!bb.isEmpty() && collisionOffset.lengthVector() > 0) {
                    for (AxisAlignedBB aabb : bb) {
                        ...
                        if (p_70091_2_ != 0.0D) {
                            ...
                            for (int l5 = bb.size(); j5 < l5; ++j5) {
                                p_70091_2_ = ((AxisAlignedBB) bb.get(j5)).calculateXOffset(aabb2, p_70091_2_);
                            }

                            if (p_70091_2_ != 0.0D) {
                                aabb2 = aabb2.offset(p_70091_2_, 0.0D, 0.0D);
                            }
                            d2 = p_70091_2_;
                        }
                        if (p_70091_6_ != 0.0D) {
                            ...
                            for (int i6 = bb.size(); k5 < i6; ++k5) {
                                p_70091_6_ = ((AxisAlignedBB) bb.get(k5)).calculateZOffset(aabb2, p_70091_6_);
                            }

                            if (p_70091_6_ != 0.0D) {
                                aabb2 = aabb2.offset(0.0D, 0.0D, p_70091_6_);
                            }
                            d4 = p_70091_6_;
                        }
                    }
            ...
            if (p_70091_4_ != 0.0D) {
                int k = 0;
                if (!ignoreVertical) for (int l = list1.size(); k < l; ++k) {
                    p_70091_4_ = ((AxisAlignedBB) list1.get(k)).calculateYOffset(this.getEntityBoundingBox(), p_70091_4_);
                }
            ...
            if (p_70091_2_ != 0.0D) {
                int j5 = 0;
                if (!ignoreHorizontal) for (int l5 = list1.size(); j5 < l5; ++j5) {
                    p_70091_2_ = ((AxisAlignedBB) list1.get(j5)).calculateXOffset(this.getEntityBoundingBox(), p_70091_2_);
                }
            ...
            if (p_70091_6_ != 0.0D) {
                int k5 = 0;
                if (!ignoreHorizontal) for (int i6 = list1.size(); k5 < i6; ++k5) {
                    p_70091_6_ = ((AxisAlignedBB) list1.get(k5)).calculateZOffset(this.getEntityBoundingBox(), p_70091_6_);
                }
            ...
            if (stepHeight > 0.0F && flag && (d2 != p_70091_2_ || d4 != p_70091_6_) && !ignoreHorizontal && !ignoreVertical) {
                double d14 = p_70091_2_;
                double d6 = p_70091_4_;
                double d7 = p_70091_6_;
            ...
            if (this instanceof EntityPlayerSP) {
                ...
                EventManager.dispatchEvent(new EventPostMove(Math.sqrt(deltaX * deltaX + deltaZ * deltaZ)));
            }
    ...
    public double getDistanceToEntityEye(Entity var1) {
        double var2 = t(this.posY + (double) this.getEyeHeight() - var1.posY, 0.0, var1.height);
        return this.getPositionEyes(1.0F).distanceTo(var1.dT().addVector(0.0, var2, 0.0));
    }
    ...
    public float getCollisionBorderSize() {
        ...
        if (Expensive.getInstance().getModuleManager().get(HitBoxModule.class).state) {
            return HitBoxModule.size.get();
        }
    ...
    public double[] razXZ(Entity entity) {
        final double polWidth = entity.width / 2;
        final double razX = MathHelper.clamp(posX - entity.posX, -polWidth, polWidth);
        final double razZ = MathHelper.clamp(posZ - entity.posZ, -polWidth, polWidth);
        return new double[]{razX, razZ};
    }

    public void setSizeAdvanced(float width, float height) {
        if (width != this.width || height != this.height) {
            float f = this.width;
            this.width = width;
            this.height = height;
