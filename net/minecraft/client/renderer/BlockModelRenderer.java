...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        try {
            ...
            if (Expensive.getInstance().getModuleManager().get(OptimizationModule.class).state
                    && OptimizationModule.grass.get()) {
                if ((stateIn.getBlock() == Blocks.TALLGRASS || stateIn.getBlock() == Blocks.DOUBLE_PLANT ||
                        stateIn.getBlock() == Blocks.DEADBUSH || stateIn.getBlock() == Blocks.YELLOW_FLOWER
                        || stateIn.getBlock() == Blocks.RED_FLOWER)) {
                    return false;
                }
            }
            return flag1;
