...
public class EntityRenderer implements IResourceManagerReloadListener {
        ...
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                ...
                for (int i = 0; i < 256; ++i) {
                    ...
                    if (Expensive.getInstance().getModuleManager().world.isWorldColor()) {
                        this.lightmapColors[i] = Expensive.getInstance().getModuleManager().world.worldColor.get();
                    }
                    else
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        if (Expensive.getInstance().getModuleManager().world.isWeather()) {
            renderRainSnowColor(partialTicks, Expensive.getInstance().getModuleManager().world.weatherColor.getColor());
        }
        ...
        EventManager.dispatchEvent(new EventDraw(EventDraw.RenderType.RENDER, partialTicks, sr));
    ...
    protected void renderRainSnowColor(float partialTicks, Color c) {
                    ...
                    Biome biome = world.getBiome(blockpos$mutableblockpos);
                        ...
                        if (k2 != l2) {
                            ...
                            float f1 = biome.getFloatTemperature(blockpos$mutableblockpos);
                                ...
                                bufferbuilder.pos((double) l1 - d3 + 0.5D, (double) l2, (double) k1 - d4 + 0.5D).tex(0.0D + d9, (double) k2 * 0.25D + d8 + d10).color(c.getRGB(), f4).lightmap(j4, k4).endVertex();
                                bufferbuilder.pos((double) l1 + d3 + 0.5D, (double) l2, (double) k1 + d4 + 0.5D).tex(1.0D + d9, (double) k2 * 0.25D + d8 + d10).color(c.getRGB(), f4).lightmap(j4, k4).endVertex();
                                bufferbuilder.pos((double) l1 + d3 + 0.5D, (double) k2, (double) k1 + d4 + 0.5D).tex(1.0D + d9, (double) l2 * 0.25D + d8 + d10).color(c.getRGB(), f4).lightmap(j4, k4).endVertex();
                                bufferbuilder.pos((double) l1 - d3 + 0.5D, (double) k2, (double) k1 - d4 + 0.5D).tex(0.0D + d9, (double) l2 * 0.25D + d8 + d10).color(c.getRGB(), f4).lightmap(j4, k4).endVertex();
                            }
                }
            }
    ...
    public void setupOverlayRendering(int scale) {
        ScaledResolution scaledresolution = new ScaledResolution(this.mc, scale);
        GlStateManager.clear(256);
        GlStateManager.matrixMode(5889);
        GlStateManager.loadIdentity();
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (Expensive.getInstance().getModuleManager().world.isFog()) {
            Color rgb = Expensive.getInstance().getModuleManager().world.fogColor.getColor();
            this.fogColorRed = rgb.getRed() / 255f;
            this.fogColorGreen = rgb.getGreen() / 255f;
            this.fogColorBlue = rgb.getBlue() / 255f;
        } else {
    ...
    private void setupFog(int startCoords, float partialTicks) {
        if (Expensive.getInstance().getModuleManager().get(NoRenderModule.class).state) {

            EventOverlay event = new EventOverlay(EventOverlay.OverlayType.Fog);
    ...
    public void func_190565_a(ItemStack p_190565_1_) {
        if (Expensive.getInstance().getModuleManager().get(NoRenderModule.class).state) {
            ...
            EventOverlay event = new EventOverlay(EventOverlay.OverlayType.TotemAnimation);
            EventManager.dispatchEvent(event);
