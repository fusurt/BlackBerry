...
import wtf.expensive.command.impl.CommandWay;
...
import wtf.expensive.modules.impl.movement.TimerModule;
import wtf.expensive.modules.impl.render.HUDModule;
import wtf.expensive.modules.impl.render.NameProtect;
import wtf.expensive.modules.impl.render.NoRenderModule;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        if (isHud) {
            if (Expensive.getInstance().config.loadConfig("default")) {
        ...
        EventOverlay event = new EventOverlay(EventOverlay.OverlayType.BossBar);
        ...
        EventManager.dispatchEvent(new EventDraw(EventDraw.RenderType.DISPLAY, partialTicks, scaledresolution));

        if (Expensive.getInstance().getModuleManager().get(Notifications.class).state) {
        ...
        if (CommandWay.way) {
            ...
            double yaw = Math.toDegrees(Math.atan2(CommandWay.z - mc.player.posZ, CommandWay.x - mc.player.posX)) - mc.player.rotationYaw
                    - 90;
            double dst = Math.sqrt(Math.pow(CommandWay.x - mc.player.posX, 2) + Math.pow(CommandWay.z - mc.player.posZ, 2));
            GL11.glPushMatrix();
            float offset = 30;
            GL11.glTranslated(x, y, 0);

            GL11.glTranslated((Math.cos(Math.toRadians(yaw - 90)) * offset), Math.sin(Math.toRadians(yaw - 90)) * offset, 0);
            GL11.glRotated(yaw, 0, 0, 1);

            Color color = ColorUtility.getColorStyleColor(1);

            RenderUtility.drawTriangle(color.getRed(), color.getGreen(), color.getBlue());

            GL11.glTranslated(0, offset / 2f, 0);
            GL11.glRotated(yaw, 0, 0, -1);
            Fonts.MONTSERRAT12.drawCenteredStringWithShadow((int) dst + "m", 1, 0, -1);
        ...
        if (Expensive.getInstance().getModuleManager().get(HUDModule.class).state) {
            float width = 20;
            float heigth = 20;
            float x = scaledresolution.getScaledWidth() / 2f - width / 2f;
            float y = scaledresolution.getScaledHeight() - 80;

        /*    if (HUDModule.blur.get())
                Utility.NORMAL_BLUR_RUNNABLES.add(() -> {
                    RenderUtility.drawRect(x, y, width, heigth, new Color(21, 21, 21, 115).getRGB());
                });

            if (HUDModule.shadow.get())
                Utility.NORMAL_SHADOW_BLACK.add(() -> {
                    RenderUtility.drawRect(x, y, width, heigth, new Color(21, 21, 21, 50).getRGB());
                });

         */
            TimerModule.animWidth = AnimationMath.fast(TimerModule.animWidth, (float) (TimerModule.getProgress()), 10.0f);
            RenderUtility.drawCircle(x + 10.5f, y - 10.5f, TimerModule.animWidth * 365.0f,  0, 10, new Color(0,0,0,75).getRGB(), 5.5F);
            RenderUtility.drawCircle(x + 10, y - 10, TimerModule.animWidth * 365.0f,  0, 10, ColorUtility.applyOpacity(new Color(Expensive.getInstance().getModuleManager().arraylist.getColor(280)), 255).getRGB(), 3.5F);
            GlowUtility.drawGlowGradient(x,  y - 20, 20, 20, 15, Expensive.getInstance().getModuleManager().arraylist.getColor(0,.3333f), Expensive.getInstance().getModuleManager().arraylist.getColor(90,.3333f), Expensive.getInstance().getModuleManager().arraylist.getColor(180,.3333f), Expensive.getInstance().getModuleManager().arraylist.getColor(240,.3333f));
            //RenderUtility.drawCircle(x + 30.5f, y - 40.5f, TimerModule.animWidth * 10000,  0, 0.5f, new Color(0,0,0,75).getRGB(), 155.5F);
            //Fonts.MONTSERRAT11.drawStringWithShadow("timer", x + 172, y + heigth - 27, -1);
            Fonts.MONTSERRAT12.drawCenteredStringWithShadow(Math.round((TimerModule.animWidth) * 100.0D) + "", x + 10, y - 11F, -1);
            //Fonts.MONTSERRAT12.drawCenteredStringWithShadow(mc.player != null && mc.player.ticksExisted % 4 < 2 ? "я таймер" : "мать ебу ля", x + 10, y + 22F, ColorUtility.rainbow(2,0,1,1,1).getRGB());
        ...
        if (!(Expensive.getInstance().getModuleManager().get(HUDModule.class).state && HUDModule.showPotionStatus.get())) {
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        if (NoRenderModule.element.get(0) && Expensive.getInstance().getModuleManager().get(NoRenderModule.class).state)
            return;
        boolean isNameProtect = Expensive.getInstance().getModuleManager().get(NameProtect.class).state;
        String protectName = CommandNameProtect.canChange ? "�6" + CommandNameProtect.current : "�6Protected";
        ...
        for (Score score1 : collection) {
            ...
            if (mc.getCurrentServerData() != null && isNameProtect) {
                ...
                if (s.contains(mc.player.getName())) {
                    s1 = s.replace(mc.player.getName(), protectName);
                }
