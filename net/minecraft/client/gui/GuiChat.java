...
import wtf.expensive.utility.math.Vec2i;
...
public class GuiChat extends GuiScreen implements ITabCompleter {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        Vec2i scale = Expensive.getInstance().getScaleMath().getMouse((int) mouseX, (int) mouseY, new ScaledResolution(Minecraft.getMinecraft()));
        ...
        DragManager.draggables.values().forEach(dragging -> {
            if (dragging.getModule().state) {
                dragging.onClick(scale.getX(), scale.getY(), mouseButton);
            }
        });
