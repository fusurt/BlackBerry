    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn)
    {
        ...
        for (int k4 = 0; k4 < l3; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                String protectName = CommandNameProtect.canChange ? CommandNameProtect.current : ChatFormatting.GRAY + "Protected";

                String s4 = this.getPlayerName(networkplayerinfo1);
                if (Expensive.getInstance().getModuleManager().get(NameProtect.class).state) {
                    if (NameProtect.youtuber.get() && !s4.contains(mc.player.getName())) {
                        s4 = ChatFormatting.GRAY + "Protected";
                    }
                    s4 = s4.replace(mc.player.getName(), protectName);
                }
