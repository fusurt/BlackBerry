...
import wtf.expensive.modules.impl.player.InventoryMoveModule;
...
public abstract class GuiContainer extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i1 = 0; i1 < this.inventorySlots.inventorySlots.size(); ++i1) {
            ...
            if (this.isMouseOverSlot(slot, mouseX, mouseY) && slot.canBeHovered()) {
                if (Expensive.getInstance().getModuleManager().get(ItemScroller.class).state) {
                    if (Mouse.isButtonDown(0) && Keyboard.isKeyDown(mc.gameSettings.keyBindSneak.getKeyCode())
                            && mc.currentScreen != null) {
                        if (timer.hasTimeElapsed((long) ItemScroller.delay.get())) {
                            GuiContainer container = (GuiContainer) mc.currentScreen;
                            container.handleMouseClick(slot, slot.slotNumber, 1, ClickType.QUICK_MOVE);
                            timer.reset();
                        }
                    }
                }
    ...
    private void drawSlot(Slot slotIn) {
        ...
        if (slotIn == this.clickedSlot && !this.draggedStack.isEmpty() && this.isRightMouseClick && !itemstack.isEmpty()) {
            ...
            if (Container.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn)) {
                ...
                flag = true;
                Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, itemstack, slotIn.getStack().isEmpty() ? 0 : slotIn.getStack().func_190916_E());
        ...
        if (!itemstack.isEmpty() && this.dragSplitting) {
            if (this.dragSplittingLimit == 2) {
                ...
                for (Slot slot : this.dragSplittingSlots) {
                    ...
                    int i = itemstack2.isEmpty() ? 0 : itemstack2.func_190916_E();
                ...
                if (this.mc.gameSettings.touchscreen) {
                    ...
                    if (this.mc.player.inventory.getItemStack().isEmpty()) {
                        if (mouseButton == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                            ...
                            if ((MoveUtility.isMoving() && this instanceof GuiInventory)
                                    && (Expensive.getInstance().getModuleManager().get(InventoryMoveModule.class)
                                    .state && InventoryMoveModule.safe.get())) {
                                return;
                            }
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick) {
        ...
        } else if (this.dragSplitting && slot != null && !itemstack.isEmpty() && (itemstack.func_190916_E() > this.dragSplittingSlots.size() || this.dragSplittingLimit == 2) && Container.canAddItemToSlot(slot, itemstack, true) && slot.isItemValid(itemstack) && this.inventorySlots.canDragIntoSlot(slot)) {
