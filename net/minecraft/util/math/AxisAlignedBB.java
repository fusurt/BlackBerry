...
public class AxisAlignedBB {
    ...
    public AxisAlignedBB offsetMXMN(double xmn, double ymn, double zmn,double xmx, double ymx, double zmx) {
        return new AxisAlignedBB(this.minX + xmn, this.minY + ymn, this.minZ + zmn, this.maxX + xmx, this.maxY + ymx, this.maxZ + zmx);
    }
    ...
    public float distanceTo(Vec3d vec) {
        float f = (float) Math.max(0.0D, this.minX - vec.x);
        float f1 = (float) Math.max(0.0D, vec.x - this.maxX);
        float f2 = (float) Math.max(0.0D, this.minY - vec.y);
        float f3 = (float) Math.max(0.0D, vec.y - this.maxY);
        float f4 = (float) Math.max(0.0D, this.minZ - vec.z);
        float f5 = (float) Math.max(0.0D, vec.z - this.maxZ);
        return MathHelper.sqrt(f * f + f1 * f1 + f2 * f2 + f3 * f3 + f4 * f4 + f5 * f5);
    }
}
