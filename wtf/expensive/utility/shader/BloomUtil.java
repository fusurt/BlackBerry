...
public class BloomUtil implements Utility {
    ...
    public static ShaderUtility gaussianBloom = new ShaderUtility("expensive/shaders/bloom.frag");
    ...
    public static void renderBlur(List<Runnable> run) {
        ...
        renderBlur(framebuffer1.framebufferTexture, 10, 1, ColorUtility.rgba(0,0,0,255), 2, true);
    ...
    public static void setupUniforms(int radius, int directionX, int directionY, FloatBuffer weights, int c, float des, boolean fill) {
        ...
        gaussianBloom.setUniformf("color", ColorUtility.getRed(c) / 255f, ColorUtility.getGreen(c) / 255f, ColorUtility.getBlue(c) / 255f);
