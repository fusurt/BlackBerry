...
@UtilityClass
public class MainPathFinder implements Utility {
    ...
    public List<Vec3> computePath(Vec3 from, final Vec3 to, final boolean exact, final double step) {
        final Vec3d blockPos = new Vec3d(from.getX(), from.getY(), from.getZ());
        final IBlockState state = mc.world.getBlockState(new BlockPos(blockPos));
    ...
    private boolean canPassThroughMaterial(final Block block) {
        final Material material = block.getMaterial(null);
