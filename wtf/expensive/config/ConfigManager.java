...
public final class ConfigManager extends Manager<Config> {
    ...
    public static final File configDirectory = new File(Minecraft.getMinecraft().mcDataDir, "\\expensive\\configs");
    ...
    private static ArrayList<Config> loadConfigs() {
        ...
        if (files != null) {
            for (File file : files) {
                if (FilenameUtils.getExtension(file.getName()).equals("cfg"))
    ...
    public void load() {
        ...
        if (configDirectory != null) {
            File[] files = configDirectory.listFiles(f -> !f.isDirectory() && FilenameUtils.getExtension(f.getName()).equals("cfg"));
            for (File f : files) {
    ...
    public Config findConfig(String configName) {
        ...
        if (new File(configDirectory, configName + ".cfg").exists())
